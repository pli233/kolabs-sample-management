...
选定一个工作表,以一个功能完整的数据表格查看 —— 支持**全列搜索、点列排序、虚拟滚动**(26 万行
服务端分页)、**列可见性多选**(默认只显示常用 12 列,可勾选任意列)、**拖拽调整列宽**、表格居中,
并对固定列 schema 给出校验提示。表格由 TanStack Table(列模型)+ TanStack Virtual(虚拟滚动)驱动。
...
```bash
cd backend && source .venv/bin/activate && python -m pytest -q   # 28 个单元/接口测试
cd frontend && npm run test                                      # 7 个组件测试
...
| GET | `/api/files/{id}/rows` | 主数据表分页行:`?offset&limit&q&sort&dir`(全量搜索/排序) |
