...
- **Dashboard** —— 针对**当前活动源**的功能完整数据表:全列搜索、按列高级筛选(包含/等于/大于/为空…
  运算符 + Match all/any 的 AND/OR 组合)、点列排序、虚拟滚动(26 万行服务端分页)、列可见性多选
  (默认 12 常用列)、拖拽列宽、居中。搜索/筛选/排序全部在后端对全量数据生效。
...
```bash
cd backend && source .venv/bin/activate && python -m pytest -q   # 34 个单元/接口测试
