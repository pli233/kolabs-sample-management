...
实验室生物样本库(biorepository)可视化管理。第一个需求:上传 Excel/CSV 库存文件并保存,
选定一个工作表,以一个功能完整的数据表格查看 —— 支持**全列搜索**、**按列高级筛选**(每列可加
包含/等于/大于/为空… 等运算符条件,多条件按「全部满足/任一满足」AND/OR 组合)、**点列排序**、
**虚拟滚动**(26 万行服务端分页)、**列可见性多选**(默认只显示常用 12 列)、**拖拽调整列宽**、
表格居中。搜索/筛选/排序全部在后端对全量数据生效。表格由 TanStack Table(列模型)+
TanStack Virtual(虚拟滚动)驱动。
...
打开前端地址 → 拖入 `data/数据库下载结果.xlsx` → 选定主数据表 → 在表格里搜索/排序。

## 测试

```bash
cd backend && source .venv/bin/activate && python -m pytest -q   # 31 个单元/接口测试
cd frontend && npm run test                                      # 8 个组件测试
...
`frontend/e2e/fixtures/sample-main-library.xlsx` 进上传区 → 在选择器里选定主数据表 → 进入 viewer
...
| GET | `/api/files/{id}/rows` | 分页行:`?offset&limit&q&filters&match&sort&dir`(全量搜索/筛选/排序) |
