...
左侧导航(所有工具都针对**当前活动源**):
- **Box Lookup** —— 输入盒号,列出它在各项目/冻存柜下的每个唯一位置 + 试管数 + 示例管(legacy
  search_box_number)。
- **QC Sampler** —— 项目 + 盒号(支持 `716-719,722` 范围)+ 每盒抽 N 管,seeded 可复现随机抽样
  (legacy make_project_box_qc_list)。
- **Aliquot Finder** —— 一批 person/project_id → 每人 PRIMARY + N BACKUP 推荐(选样最多的冻存柜,
  可指定优先柜)(legacy find_person_aliquots)。
- **Scan Reconcile** —— 上传实物扫描文件(.csv/.xlsx/.xls)与活动源对账:错码/放错位/漏扫/位置冲突,
  自动去重重复扫描文件(legacy compare_scan_to_database)。
...
data/      sample_database.xlsx —— 本需求的样本/测试夹具
...
打开前端地址(`/` 自动跳 Dashboard)→ 进 **Data Feeds** 拖入 `data/sample_database.xlsx`
→ 选定主数据表 → 它成为活动源,回 **Dashboard** 搜索/筛选/排序。

## 测试

```bash
cd backend && source .venv/bin/activate && python -m pytest -q   # 57 个单元/接口测试
cd frontend && npm run test                                      # 10 个组件测试
cd frontend && npm run e2e                                       # Playwright 端到端
```

后端测试用真实样本 `data/sample_database.xlsx` 作夹具;上传用例会完整解析 26 万行,故整套约 3-4 分钟。
...
→ 搜索/筛选/排序/导出 → 遍历 6 个侧边栏 tab(Dashboard/Box Lookup/QC Sampler/Aliquot Finder/Scan Reconcile/Data Feeds)可达;另含不支持类型被拒的场景。
...
| GET | `/api/files/{id}/overview` | 活动源聚合(图表用):总数/按冻存柜/按项目 |
| DELETE | `/api/files/{id}` | 删除 feed;若是活动源则改派最近的一个 |
| GET | `/api/files/{id}/export` | 当前视图导出带样式 xlsx(同 rows 参数 + `columns`) |
| GET | `/api/box-lookup` | `?box=&format=json\|xlsx` 盒号→位置+示例管 |
| GET | `/api/qc-sample` | `?project=&boxes=&per_box=&seed=&format=` seeded QC 抽样 |
| GET | `/api/aliquot-finder` | `?ids=&preferred_freezer=&backups=&format=` PRIMARY/BACKUP |
| POST | `/api/scan-reconcile` | 上传扫描文件(multipart)对账;`format=json\|xlsx` |
